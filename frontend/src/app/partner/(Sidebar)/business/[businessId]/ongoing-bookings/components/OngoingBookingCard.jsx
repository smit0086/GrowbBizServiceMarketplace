...
const OngoingBookingCard = ({ authSession, ongoingBooking }) => {
    const [isCompletedConfirmationDialogOpen, setCompletedConfirmationDialogOpen] = useState(false);
    const { handleSubmit, control, formState: { errors } } = useForm();
    const [isLoading, setIsLoading] = useState(false);

    const handleCompletedStatus = async (data) => {
        setIsLoading(true);
        await updateBookingStatus(authSession.apiToken, data.ongoingBooking.id, BOOKING_STATUS.COMPLETED);
        setIsLoading(false);
